...
(function () {
    ...
    $(document).ready(function () {
        myJpopup = $("#any").jpopup({width: 350, height: 140, target: $("#getTicket"), speed: 20});
        var d;
            ...
            if (!$(this).hasClass("cur")) {
                if (f < 6) {
                    if ($.canSelectThisSeatType(g)) {
                        ...
                        this.checked = false;
                        dhtmlx.alert({title: "添加席别", ok: "确定", text: "学生不可以购买" + h + "。", type: "alert-error"})
                    }
                } else {
                    dhtmlx.alert({title: "添加席别", ok: "确定", text: "最多添加5个优先席位", type: "alert-error"})
                }
            } else {
        ...
        $("#date-list li").click(function () {
            ...
            if (!$(this).hasClass("cur")) {
                if (h < 6) {
                    ...
                    dhtmlx.alert({title: "添加备选日期", ok: "确定", text: "最多添加5个备选日期", type: "alert-error"});
                    $(this).removeClass("cur")
                }
            } else {
        ...
        $("#auto_query").mouseenter(function (h) {
            var f = h.pageY + 10;
            var g = h.pageX;
            $("#showquick1").css({top: f, left: g - 96});
            $("#showquick1").show()
        });
...
(function (b, e, c) {
    ...
    c[b] = {
        ...
        settings: {
            ...
            flashError: function () {
                var f = this.settings.createPlayer, g = a(f.errorMessageClass, this.wrapper),
            ...
            loadStarted: function () {
                var f = this.settings.createPlayer, g = a(f.durationClass, this.wrapper),
                    i = Math.floor(this.duration / 60), h = Math.floor(this.duration % 60);
                c[b].helpers.removeClass(this.wrapper, f.loadingClass);
                g.innerHTML = (i < 10 ? "0" : "") + i + ":" + (h < 10 ? "0" : "") + h
            },
            loadProgress: function (f) {
                var g = this.settings.createPlayer, h = a(g.scrubberClass, this.wrapper);
        ...
        createAll: function (f, g) {
            var l = g || document.getElementsByTagName("audio"), k = [];
            f = f || {};
        ...
        newInstance: function (f, g) {
            var j = this.helpers.clone(this.settings), i = "audiojs" + this.instanceCount,
        ...
        createPlayer: function (f, g, j) {
            var i = document.createElement("div"), h = f.cloneNode(true);
        ...
        attachEvents: function (f, g) {
            if (g.settings.createPlayer) {
                var j = g.settings.createPlayer, i = a(j.playPauseClass, f), h = a(j.scrubberClass, f);
        ...
        injectFlash: function (f, g) {
            ...
            var i = f.wrapper.innerHTML, h = document.createElement("div");
            h.innerHTML = j + i;
        ...
        helpers: {
            merge: function (f, g) {
                ...
                var g = document.createDocumentFragment(), h = g.createElement ? g : document;
        ...
        events: {
            memoryLeaking: false, listeners: [], addListener: function (f, g, h) {
...
(function (a) {
    ...
    a.jpopup.defaultOptions = {width: 180, height: 100, target: null, speed: 20};
...
(function (a) {
    ...
    a.autoSubmit.defaultOptions = {partSubmit: "partSubmit", allSubmit: "autoSubmit", prior: "_prior"};
